...
class GamesWindow {
    ...
    startButtonGame() {
        const noBtn = document.querySelector('#gameBoard-button .no-btn');
        const yesBtnContainer = document.querySelector('#gameBoard-button .button-group');
        let attempts = 0;
        ...
        noBtn.addEventListener('click', (e) => {
            ...
            if (attempts > 5) {
                const message = document.querySelector('#gameBoard-button .button-message');
                message.classList.remove('hidden');
                message.textContent = 'You can\'t click No! You\'re stuck with me forever! 💕';
                noBtn.style.pointerEvents = 'none';
                playSound('success');
            }
        });

        document.querySelector('#gameBoard-button .yes-btn').addEventListener('click', () => {
            playSound('success');
            alert('I love you too! Forever my Valentine! 💕');
            config.gameScores['button'] = 100;
            saveData();
            createConfetti(window.innerWidth / 2, window.innerHeight / 2, 50);
            unlockAchievement('games_won', 'Game Master', '🏆');
        });
    }
}
